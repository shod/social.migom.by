 ...
 * @since v1.0
 */
abstract class ERestDocument extends ERestEmbeddedDocument
{
...
//	}
    ...
    public function __call($function, $args)
    {
        ...
        $result = $this->query($function, $id, $method, $params);
        if(isset($result->content)){
            if(isset($result->content->result)){
	...
	/**
	 * Get RestServer component instance
	 * By default it is 'restServer' application component
	 *
	 * @return RestServer
	 * @since v1.0
	 */
	...
	{
		if(self::$_restServer===null)
			self::$_restServer = Yii::app()->getComponent('restServer');

		return self::$_restServer;
	}

	/**
	 * Set ERestServer component instance
	 * @param ERestServer $component
	 * @since v1.0
	 */
	public function setRestComponent(ERestServer $component)
	{
		return self::$_restServer = $component;
	}

	/**
	 * Get raw ERestServer instance
	 * @return ERestServer
	 * @since v1.0
	 */
	public function getServer()
	{
		return $this->getRestComponent()->getServerInstance();
	}
	...
	public function getCollection()
	{
		if(!isset(self::$_collections[$this->getCollectionName()])){
            self::$_collections[$this->getCollectionName()] = $this->getServer(); //->selectCollection($this->getCollectionName())
        }
	 ...
	 * @return ERestCriteria the query criteria that is associated with this model.
	...
	public function getDbCriteria($createIfNull=true)
	{
		if($this->_criteria===null)
			if(($c = $this->defaultScope()) !== array() || $createIfNull)
				$this->_criteria = new ERestCriteria($c);
	 ...
	 *
	 * @param ERestCriteria $criteria
	 * @since v1.0
	 */
	public function setDbCriteria($criteria)
	{
		if(is_array($criteria))
			$this->_criteria = new ERestCriteria($criteria);
		else if($criteria instanceof ERestCriteria)
			$this->_criteria = $criteria;
		else
			$this->_criteria = new ERestCriteria();
	}
	...
	public function getUseCursor()
	{
		...
		return $this->getRestComponent()->useCursor;
	}
	 ...
	 * @param ERestCriteria|array $criteria the query criteria. This parameter may be modified by merging {@link dbCriteria}.
	 * @since v1.2.2
	 */
	public function applyScopes(&$criteria)
	{
		if($criteria === null)
		{
			$criteria = new ERestCriteria();
		}
		else if(is_array($criteria))
		{
			$criteria = new ERestCriteria($criteria);
		}
		else if(!($criteria instanceof ERestCriteria))
	...
	public function insert(array $attributes=null)
	{
		...
		if($this->beforeSave())
		{
            ...
            $result = $this->query(__FUNCTION__, null , 'post', $rawData);

			if($result !== false && isset($result->content->result)) // strict comparison needed
			{
                $this->id = $result->content->result;
	...
	{
		if($this->getIsNewRecord())
			throw new ERestException(Yii::t('yii','The ERestDocument cannot be updated because it is new.'));
		if($this->beforeSave())
		{
			Yii::trace(get_class($this).'.update()','ext.Rest.ERestDocument');
			$rawData = $this->toArray();
			// filter attributes if set in param
			if($attributes!==null)
			{
				if (!in_array('id', $attributes) && !$modify) $attributes[] = 'id'; // This is very easy to forget
            ...
            $result = $this->query(__FUNCTION__, null , 'put', $rawData);
			...
			throw new ERestException(Yii::t('yii', 'Can\t save document to disk, or try to save empty document!'));
		}
	}
	/**
	 ...
	 * @param ERestCriteria $criteria condition to limit updating rules
	 * @return bool
	 */
	 ...
	 * @param array|ERestCriteria $condition query criteria.
	 * @since v1.0
	 */
    ...
    public function query($function = '', $id = null, $method = 'get', $params = array())
    {
        $uri = $this->_createUri($function, $id);
        Yii::trace(get_class($this) . '.query()', 'RESTClient');
        $responce = $this->getCollection()->{$method}($uri, $params, 'json');
        if($this->debug == true){
            $this->getCollection()->debug();
        }
//        $this->getCollection()->debug();
        if(!is_object($responce)){
            $this->addError('', $responce);
            return false;
        }

        if($responce->content->success !== true){
            $this->addError('', $responce->content->message);
            Yii::log($responce->content->message, CLogger::LEVEL_ERROR, 'api_client');
        }
        return $responce;
    }

    protected function _createUri($function = '', $id = null) {
        $uri = $this->getCollectionName();
        if ($function) {
            $uri .= '/' . $function;
        }
        if ($id) {
	 ...
	 * @since v1.0
	 */
	public function find($criteria=null, $type = 'find')
	{
		...
		if($this->beforeFind())
		{
			...
			$doc = $this->query($type, null, 'get', array('criteria' => $criteria->getConditions()));
	...
	{
		Yii::trace(get_class($this).'.findAll()','ext.Rest.ERestDocument');
	...
	{
		Yii::trace(get_class($this).'.findByPk()','ext.Rest.ERestDocument');
		if($this->beforeFind())
		{
			$doc = $this->query(__FUNCTION__, $pk, 'get');
			return $this->populateRecord($doc);
		}
	}
	...
	{
		Yii::trace(get_class($this).'.findAllByPk()','ext.Rest.ERestDocument');
		$criteria = new ERestCriteria($criteria);
	...
	protected function instantiate($attributes)
	{
        ...
        $attr = get_object_vars($attributes);
		$model->setAttributes(get_object_vars($attributes), false);
		return $model;
	}
	...
	{
		if($document!==null && $document->content->success === true)
		{
			$model=$this->instantiate($document->content);
	...
	private function createPkCriteria($pk, $multiple=false)
	{
		...
		else if(is_array($pkField))
		{
			if(!$multiple)
				...
				throw new ERestException(Yii::t('yii', 'Cannot create PK criteria for multiple composite key\'s (not implemented yet)'));
		}
